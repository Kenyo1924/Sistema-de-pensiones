...
$query_users = "CREATE TABLE IF NOT EXISTS users (
    ...
    role ENUM('superadmin', 'editor', 'viewer') NOT NULL,
    ...
    echo "Table 'student_semester_progress' created successfully.<br>";
        ...
        try {
            ...
            if ($fields && $fields->rowCount() > 0) {
                // Migrar datos antiguos mapeando a nuevos tipos
                $db->exec("INSERT INTO student_programs (student_id, program_type, is_scholarship_holder)
                  SELECT id, 
                    CASE 
                      WHEN program_type = 'maestria' THEN 'maestria_gestion_educativa'
                      WHEN program_type = 'doctorado' THEN 'doctorado'
                      ELSE 'maestria_gestion_educativa'
                    END,
                    is_scholarship_holder 
                  FROM students WHERE program_type IS NOT NULL");
        ...
        // Agregar o actualizar columna program_type en payments
        $field4 = $db->query("SHOW COLUMNS FROM payments LIKE 'program_type'");
        if ($field4 && $field4->rowCount() == 0) {
            $db->exec("ALTER TABLE payments ADD COLUMN program_type ENUM('maestria_gestion_educativa','maestria_educacion_superior','maestria_psicologia_educativa','maestria_ensenanza_estrategica','doctorado') NOT NULL DEFAULT 'maestria_gestion_educativa' AFTER semester_id");
            ...
            echo "Columna program_type actualizada en payments.<br>";
        }
        
        // Eliminar columna voucher_path si existe
        $field5 = $db->query("SHOW COLUMNS FROM payments LIKE 'voucher_path'");
        if ($field5 && $field5->rowCount() > 0) {
            $db->exec("ALTER TABLE payments DROP COLUMN voucher_path");
            echo "Columna voucher_path eliminada de payments.<br>";
        }
        
        // Migrar datos existentes de maestría a maestría_gestion_educativa
        try {
            ...
            echo "Datos existentes migrados de maestría a maestría_gestion_educativa.<br>";
        } catch (Exception $e) {
            echo "Error migrando datos existentes: ".$e->getMessage()."<br>";
        }
        
        // Actualizar tablas existentes para nuevos tipos de maestría
        try {
            ...
            echo "Error actualizando student_programs: ".$e->getMessage()."<br>";
        }
        ...
        try {
            ...
            echo "Error actualizando student_semester_progress: ".$e->getMessage()."<br>";
        }
    } catch (Exception $e) {
        echo "Error migrando programas: ".$e->getMessage()."<br>";
    }

    // Migrar tabla users para incluir superadmin si no existe
    try {
        $roleColumn = $db->query("SHOW COLUMNS FROM users LIKE 'role'")->fetch(PDO::FETCH_ASSOC);
        if ($roleColumn && strpos($roleColumn['Type'], 'superadmin') === false) {
            echo "Actualizando tabla users para incluir rol superadmin...<br>";
            $db->exec("ALTER TABLE users MODIFY COLUMN role ENUM('superadmin', 'editor', 'viewer') NOT NULL");
            echo "Tabla users actualizada correctamente.<br>";
        } else {
            echo "Tabla users ya incluye el rol superadmin.<br>";
        }
    } catch (Exception $e) {
        echo "Error actualizando tabla users: ".$e->getMessage()."<br>";
    }

    // Insert default users
    $superadmin_password = password_hash('superadmin123', PASSWORD_BCRYPT);
    $editor_password = password_hash('editor123', PASSWORD_BCRYPT);
    $viewer_password = password_hash('viewer123', PASSWORD_BCRYPT);

    $stmt = $db->prepare("INSERT INTO users (username, password, role) VALUES ('superadmin', :password, 'superadmin') ON DUPLICATE KEY UPDATE password=:password");
    $stmt->bindParam(':password', $superadmin_password);
    $stmt->execute();
